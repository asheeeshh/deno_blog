...
function PostCard({ post, timezone }: { post: Post; timezone: string }) {
  return (
    <div class="pt-12 first:pt-0">
      ...
      <p class="mt-3">
        <a
          class="leading-tight text-gray-300 inline-block border-b-1 border-gray-300 hover:text-gray-200 hover:border-gray-200 transition-colors"
          href={post.pathname}
      ...
      <article>
        ...
        <p class="mt-1 text-gray-300">
          {(state.author || post.author) && (
            <span>
              by {state.author || post.author} on {" "}
            </span>
          )}
...
function Footer(props: { author?: string }) {
  return (
    <footer class="mt-20 pb-16 lt-sm:pb-8 lt-sm:mt-16">
      <p class="flex items-center gap-2.5 text-gray-300 text-sm">
        <span>
        ...
        <a
          href="/feed"
          class="inline-flex items-center gap-1 hover:text-gray-200 transition-colors"
          title="Atom Feed"
        >
          <IconRssFeed /> RSS
        </a>
